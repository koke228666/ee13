/*
vk.al = -1 - (обычная загрузка страниц через фрейм) ответ(вся реализация) во фрейме.
vk.al = 1 - (обычный ajax запрос) ответ через сплиты "<!>".
vk.al = 2 - если браузер не поддерживает history.pushState (). 
vk.al = 3 - нормальная работа, (сгенерированная сервером) загрузка страницы.
vk.al = 4 - перезагрузка страницы.
*/
if (vk.al == 1) { // Перезагружает страницу. 
...
// cur.destroy - выполнения функций в destroy при переходе через nav.go.
// cur.nav - выполнения функций в nav при при переходе через nav.go (для перехвата запроса на текущ. стр.).
var cur = {destroy: [], nav: []};

// Cоответствие браузера, взрощает true если браузер совподает, иначе false
var browser = {
...
// Кроссбраузерная инсталляция флеш контейнера.  
(function() {
  ...
  if (navigator.plugins && navigator.mimeTypes.length) { // Версия флеша в браузере. 
...
// Установка куков с размерами экрана для андроидов.
if (browser.android) {
...
// Удаление статических файлов, если зашли не с IE.
if (!browser.msie6) {
...
// Инсталляция, преобразование имени статик. файла для id div-ов в utils. 
for (var i in StaticFiles) {
  var f = StaticFiles[i];
  f.t = (i.indexOf('.css') != -1) ? 'css' : 'js'; 
  f.n = i.replace(/[\/\.]/g, '_'); //превращает "." в "_"
  f.l = 0;
  f.c = 0;
}
...
window.__dev = true;///[a-z0-9_\-]+\.[a-z0-9_\-]+\.[a-z0-9_\-]+\.[a-z0-9_\-]+/i.test(locHost);
if (!__dev) __debugMode = false;
window.locHash = location.hash.replace('#/', '').replace('#!', ''); // Убирает #/ и #!.
window.locBase = location.toString().replace(/#.+$/, ''); // Убирает хэш теги.
...
// Всплывающее окно ошибки сверху.
...
// Ищет значение в cur.lang, lang, langpack и возвращает найденое значение. 
function getLang() {
  try {
...
function debugLogHist(msg){
  try {
    ...
    if (ge('debuglog')) {
      ...
      _debugLogHistOffset++;
      if (_debugLogHistOffset >= 20) { // Удаления лога с начала массива. (От переполнения массива) 
...
// Подобие $() jquery, возвращает первый элемент
// В опере работает быстрее чем getElementsByTagName 
function geByTag1(searchTag, node) {
  node = node || document;
  return node.querySelector && node.querySelector(searchTag) || geByTag(searchTag, node)[0];
}

// Возвращает один или несколько элементов по searchClass, 
// Если передан tag то вернет один элемент по tag.
...
// Вернет только один элемент по searchClass и tag.
...
// Создает и добавляет в DocumentFragment проанализированный html.
// Работает быстрее чем (цикличная) обычная вставка данных в ноду.
...
// Возвращает первый элемент передановго html
function se(html) {return ce('div', {innerHTML: html}).firstChild;}

// Ищет и заменяет ключи передановго масива на значеня в строке.  
...
// Заменяет старые ссылки хранилище данных на новые
function psr(html) {
...
// Проверяет не скрыт ли елемент, вернет bool.
...
// Переключатель.
// Если элемент скрыт - показывает, если открыт - скрывает.
...
// Скрывает или показывает все флеш контейнера. 
var hfTimeout = 0;
...
// Возвращает положения элемента по XY.
...
// Возвращает размер елемента, если передан 2 пар. отнимает границы(padding,border).
...
// Возвращает маштаб. На сколько увеличина страница. 1 = 100%.
function getZoom() {
...
// Вызывает текущую функцию с контекстом window и передают текущие параметры.
...
function vkNow() { return +new Date; } // Unix Time.
function vkImage() { return window.Image ? (new Image()) : ce('img'); } // IE8 workaround.
function trim(text) { return (text || '').replace(/^\s+|\s+$/g, ''); } // Вырезает пробелы
function stripHTML(text) { return text ? text.replace(/<(?:.|\s)*?>/g, '') : ''; } // Вырезает все html теги
function escapeRE(s) { return s ? s.replace(/([.*+?^${}()|[\]\/\\])/g, '\\$1') : ''; } // Экранирует []/(){}.+?|*
...
// Заменяет код html на обычные скобки и тд.
...
// Возвращает текст заменяя html разметку.
function replaceEntities(str) {
  return se('<textarea>' + (str.replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;')) + '</textarea>').value;
}

// Замена html разметки
function clean(str) {
...
// Возвращает позицию значеня в массиве.
...
// Возвращает bool тип, проверка на наличия значеня в массиве.
function inArray(value, arr) {
  return indexOf(arr, value) != -1;
}

// Клонирует массив, объект.
...
// Слив объектов в один (переданный первичным) 
// Если первый пар. true то переберает рекурсией вложенность
function extend() {
...
// Наличие атрибуда Class.
function hasClass(obj, name) {
  obj = ge(obj);
  return obj && (new RegExp('(\\s|^)' + name + '(\\s|$)')).test(obj.className);
}

// Добавляет атрибут class или добовляет к class значение
...
// Если class есть - удаляет, нету - создает
...
// Для совместимости событий
// Запись и чтение vkCache.
...
// Удаляет элементы массива, el - массив, остальные аргументы - элементы которые надо удалить 
...
// Отчищает ячейку vkCache, и удаляет событие btnevents с элемента
function cleanElems() {
  var a = arguments;
...
// Возвращает объект с нужным Fx.Attrs элементом. 
...
// Сливает в window стандартные функции для анимации.
...
// Основной механизм анимации. Счет времени, промежутков и т.д
Fx.Base.prototype = {
...
// Возвращает цвет переведеный в RGB [255,255,255]
...
// Возвращает цвет элемента в формате RGB[255,255,255]
...
// Скролинг к началу страницы.
function scrollToTop(speed) {
  return scrollToY(0, speed);
}

// Возвращает место положения скролла по Y. 
function scrollGetY() {
  return window.pageYOffset || scrollNode.scrollTop || document.documentElement.scrollTop;
}

// Ставит цвет у элемента на 4 секунды запоминая предыдущий цвет в кеше.
...
// Установка атрибута title элементу.
...
// Заменяет действующее событие.
// Ставит type и target у события.
// type - тип события. target - элемент у которого произошло событие.
...
// Перехватывает событие на всплытие и отключает реакцию браузера. 
...
// Для перехвата событий на всплытие. 
...
// Делает Кросс-браузерность события
...
// Отчистка от событий не связанных с window
...
// Вывод инфы о всех элементах - событий. 
if (__debugMode) {
...
var layers = {
  ...
  visible: false,
  _show: function(el, con, opacity, color) { // Показывает слой и блок с контентом
    ...
    if (con.visibilityHide) {
      ...
      show(con); // Показывает див с контентом
    }
    layers.sh(el);
    window.updateWndVScroll && updateWndVScroll();
  },
  _hide: function(el, con) { // Скрывает слой и блок с контентом
    var done = function() {
      if (con && con.visibilityHide) {
        addClass(con, 'box_layer_hidden');
      } else {
        hide(con); // Скрывает див с контентом
      }
...
// Обновления при скролле 
window.__scrLeft = 0;
function updSideTopLink(resized) {
  if (!window.scrollNode || browser.mobile) return;

  var pb = ge('page_body'), // Контент (без шапки и меню).
      xy = getXY(pb), // xy контента 
      st = scrollGetY(), // На сколько страница прокручена от вверха
      scl = bodyNode.scrollLeft, // на сколько страница прокручена влево 
      sb = ge('side_bar'), // Левая часть (Меню)
      is = isVisible(sb); // Есть ли левая часть (Меню)
  
  // Размер от вверха контента + сайтбар(меню) - позиция скролла
  // Вернет 0 если страница проскролится ниже блока с меню, 
  // иначе оставшийся число скролла до конца блока с меню.  
  window._stlSideTop = Math.max(xy[1] - (browser.mozilla ? getXY(pageNode)[1] : 0) + (is ? getSize(sb)[1] : 0) - (_fixedNav ? 0 : st), 0);

  // Установка блоку "наверх" и кнопкам позиции в зависимости от скролла. 
  if (resized || scl != __scrLeft) {
    var pl = ge('page_layout'), // весь контент
        // vk.rtl если true то блок "наверх" будет справа. 
        l = vk.rtl ? pl.offsetLeft + pl.ge('page_layout').offsetLeft  : 0, // отступ слева + отступ слева или 0 
        w = vk.rtl ? (lastWindowWidth - l) : pl.offsetLeft; // общаяя ширина - отступ слева или отсуп слева
  ...
  // Установка высоты и ширины блоку с кнопками. 
  setStyle(_stlSide, {
    top: _stlSideTop,
  ...
  var mx = 200, // позиция скролла
      vis = _tbLink.loc || _stlWas || (st > mx), // если есть сслыка назад или тек. позиция скролла больше 200 то true
      o = 0, 
      f = false, 
      regBar = (st > 250) && cur._regBar; // true если тек.скролл больше 250 или есть регБар
...
function updGlobalPlayer() {
  ...
  if (gpL + gpW <= dwidth && !vk.rtl || vk.rtl && gpL >= 0) {
    ...
    gpAbs = !headFixed && scrollGetY() < 29;
    if (browser.mac) {
      var gpTop = gpAbs || headFixed ? 49 : 20;
      setStyle(_gp, {top: gpTop});
      toggleClass(_gp, 'abs', gpAbs);
    } else {
      var gpTop = headFixed ? 49 : Math.max(Math.min(49 - scrollGetY(), 49), 20),
...
function onBodyResize(force) {
  ...
  updSideTopLink(1);
  // Для ресайза всех открытых боксов 
...
// Обновления левой части, плеера и pads.
...
function onDocumentClick(e) {
  ...
  var i = 4, target = e.target || e.srcElement, href, path, hash, w = window;
...
// Предварительная установка icon.
(function() {
...
// Проверка события у блока "наверх".
function _stlClick(e) {
  return checkEvent(e) || cancelEvent(e);
}

// Нажатия кнопки "наверх" - скроллинг.
...
// Кодирует из массива в валидный URL 
function ajx2q(qa) {
...
// Декодирует из URL в массив с валид значениями.   
function q2ajx(qa) {
  if (!qa) return {};
...
// Загрузка статик. файлов с возможностью передачи обработчика. 
var stManager = {
  _waiters: [],
  _wait: function() {
...
var globalAjaxCache = {};
var ajax = {
  _init: function() {// Кроссбраузерная установка ajax-а в _req.
    var r = false;
    try {
  ...
  _getreq: function() {// Возвращает экземпляр ajaxa, если его нету, создает.
  ...
  _receive: function(cont, html, js, bench, params) {
    ...
    if (params && 'leftads' in params) {
      __adsSet(params.leftads, params.ads_section || '', params.ads_can_show);
    }
    ajax._framenext();
  },
  framedata: false,
...
// Кроссбраузерная работа с url. 
// Изменение, установка url, и получение установленного url.
...
// Инициализация и определения обработчика на изменения url.
window.hab = new HistoryAndBookmarks({onLocChange: function(loc) {
  // hist - скрытие всех лейеров. 
  // back - для вывода кэша страницы из globalHistory.  
  nav.go('/' + loc, undefined, {back: true, hist: true});
}});

// Проверка события на клик с зажатой клавишей. 
...
// onmouseover на блок с напоминаниями.
...
// onmouseout на блок с напоминаниями.
...
// Старые функции для работы с друзьями. Сейчас все в profile.js 
...
// Сторонние сервисы сбора статистики.
...
// Подготовка. 
// Сохранение параметров, установка размеров страницы и установка счетчиков меню.
vk.counts = {};  
function handlePageParams(params) {
  ...
  // Установка счетчиков в vk.counts.
  ...
  // Установка счетчиков приложениям. 

  // Перебор элементов после сепаратора. 
  ...
  // Если есть елементы после сепаратора то показать сепаратор, иначе скрыть. 
  (sh ? show : hide)(sep);
...
// Выполнение hide и destroy функций у cur._back 
...
// Удаление объекта из globalHistory если текущ. url совпадает с тем, который в истории. 
var globalHistory = [];
...
// Проверка на частоту перезагрузки страниц, 
// если кол-во перезагрузки более 5 раз и менее 20 сек. 
// то выдаст ошибку и вернет true иначе false
...
var nav = {
  getData: function(loc) { // Получение данных роутинга. 
    if (loc.length) {
      for (var i in navMap) {
        if (i[0] == '<') continue;
  ...
  reload: function(opts) {
    if (reloadCheckFlood(opts)) return; // Проверка на частоту перезагрузок. 
    opts = opts || {};
    if (opts.force) { // Полная перезагрузка. 
      hab.stop();
      location.href = '/' + nav.strLoc;
    } else { // Перезагрузка контента без (перехвата) cur.nav и zNav. 
      nav.go('/' + nav.strLoc, undefined, extend({nocur: true}, opts));
    }
  },
  go: function(loc, ev, opts) {
    if (checkEvent(ev)) return; // Не выполнять, если был не одиночный клик.

    // Подготовка. (дэкод - энкод текущ. url).
    ...
    if (!opts.nocur) {
      // Синхронизация текущ. url с уже установленным. 
    ...
    // Выполнения текущих cur.nav функций (перехват запроса на текущ. стр.).
    ...
    if (opts.back) {
      // Выполнение onBack для перехвата вывода при нажатии назад. 
      if (cur._back && cur._back.onBack) {
        return cur._back.onBack();
      }
      // Вывод из истории если url в истории совпадает с переданным(текущ).
      for (var i = 0, l = globalHistory.length; i < l; ++i) {
    ...
    var done = function(title, html, js, params) {
      ...
      // Сохранение парам., установка размера страницы и установка счетчиков в меню.
  ...
  setLoc: function(loc) { // Установка url.
    if (typeof(loc) == 'string') {
      nav.strLoc = loc;
  ...
  change: function(loc, ev, opts) { // Запрос текущей страницы с переданными парам. 
  ...
  fromStr: function(str) { // Разбивает строку запроса на объект.
    str = str.split('#');
    var res = str[0].split('?');
    var param = {'0': res[0] || ''}
    if (str[1]) {
  ...
  toStr: function(obj) { // Объект запроса превращает в строку. 
    obj = clone(obj);
    var hash = obj['#'] || '';
    var res = obj[0] || '';
    delete(obj[0]);
  ...
  init: function() { // Получение и сохранение в поля объекта nav текущего url. 
...
// Установка или получнене значений из контейнеров или полей. 
...
// Установка фокуса и указателя элементу. 
...
// --- Боксы разобраны 30.05.13. Нужно прокомментировать!!!
// Message box
var _message_box_guid = 0, _message_boxes = [], _show_flash_timeout = 0;

var __bq = boxQueue = { // Очередь. 
  hideAll: function() { // Закрывает все боксы. 
    if (__bq.count()) {
  ...
  hideLast: function(check, e) { // Закрывает текущий бокс. 
    if (__bq.count()) {
  ...
  hideBGClick: function(e) { // Клик по слою.
    if (e && e.target && /^box_layer/.test(e.target.id)) {
      __bq.hideLast();
    }
  },
  count: function() { // Кол-во боксов в очереде. 
    return __bq._boxes.length;
  },
  _show: function(guid) { // Показывает бокс определяя очредь.  
  ...
  _hide: function(guid) { // Закрывает бокс. Если в очереди есть боксы, то открывает предыдущий.
    var box = _message_boxes[guid];
...
<div class="box_controls_text"></div>\
  ...
  var retBox = _message_boxes[guid] = {
    ...
    evalBox: function(js, url, params) {
      var scr = '((function() { return function() { var box = this; ' + (js || '') + ';}; })())'; // IE :( Пичаль и безысходность...
      if (__debugMode) {
        var fn = eval(scr);
...
// Установка автоматической длины textarea при заполнении поля. 
...
function showWriteMessageBox(e, id) {
  ...
  stManager.add(['mail.css', 'wide_dd.css', 'page.css', 'page.js', 'wide_dd.js', 'writebox.js']);
  var box = showBox('al_mail.php', {act: 'write_box', to: id}, {stat: ['mail.css', 'wide_dd.css'], cache: 1}, e);
  ...
  if (zType) {
    ...
    switch (zType[1]) {
      ...
      case 'albums':
        ...
        break;
      case 'album':
        showAlbum(zType[2], extend(opts || {}, {onFail: onFail, noHistory: true}));
        return false;
        break;
      case 'video':
