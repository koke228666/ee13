var Friends = {
  init: function(obj, friendsTpl, commonTpl) {
    ...
    cur.nav.push((function(changed, old, n) {
      if ('id' in changed || 'sort' in changed || isEmpty(changed)) {
        return true;
      }
    ...
    if (cur.silent) {
      ...
      ajax.post('al_friends.php', {act: 'load_friends_silent', id: cur.oid, gid: cur.gid, sort: nav.objLoc.sort}, {
        onDone: (function(data, universities, filters) {
  ...
  showMore: function(clear, start, end, plain) {
    ...
    for (i in friends) {
      ...
      if (cur.importCount && cur.importCount == cur.shownFriends && cur.section == 'suggestions') {
  ...
  showSection: function(section) {
    ...
    if (vk.id == cur.oid) {
      if (cur.section == 'requests' || cur.section == 'all_requests' || cur.section == 'out_requests') {
        ...
        (cur.allFriendsCount ? removeClass : addClass)(ge('main_class'), 'wide');
      }
    ...
    cur.selection = false;
    if (cur.filter && !cur.filterIds && section != 'suggestions') {
  ...
  changeSummary: function() {
    ...
    } else if (cur.section == 'suggestions') {
      if (!cur.importCount) {
        html = cur.summaryLang['suggestions'];
      } else {
        html = cur.summaryLang['import'];
      }
  ...
  section: function(type, callback, updateData) {
    ...
    if (cur.silent && type != 'suggestions' && type != 'out_requests') {
      cur.onSilentLoad = function() {
    ...
    if (!updateData && cur.friendsList[type] || type == 'all' || (type == 'requests' && cur.friendsList['sugg_requests'])) {
      if (!cur.filter || type != 'suggestions') {
    ...
    switch (type) {
      ...
      case 'recent':
        ...
        break;
      case 'suggestions':
        this.loadingState('suggestions');
        ajax.post('/friends', extend({
          act: 'get_section_friends',
          section: 'suggestions'
        }, cur.filter || {}), {
          onDone: (function(list, preload, data) {
            var list = eval('('+list+')');
            extend(cur, data || {});
            var keyType = type + (cur.filterIds ? '_filter' : '');
            cur.friendsList[keyType] = list[type];
            this.loadingState(type, true);
            this.showSection(type);
            cur.secData[type].preload = preload;
            Friends.onSectionChange();
            callback();
          }).bind(this)
        });
      ...
      case 'out_requests':
        ...
        cur.fContent.innerHTML = '<div class="wide_loading"></div>';
        ajax.post('/friends', {act: 'out_requests'}, {onDone: (function(data, universities) {
          var obj = eval('('+data+')');
          ...
          extend(cur.universities, universities);
  ...
  drawFriend: function(friend, first, type) {
    if (cur.section == 'requests' || cur.section == 'all_requests' || cur.section == 'out_requests' || cur.section == 'suggestions') {
  ...
  onRemoveFriend: function(mid, res) {
    ...
    for (var i in cur.friendsList) {
      if (i != 'all' && i != 'requests' && i != 'all_requests' && i != 'out_requests' && i != 'suggestions') {
  ...
  showCommonBox: function(e, id, hash) {
    ...
    showBox('al_friends.php', {act: 'common_friends', mid: id, hash: hash}, {params: {bodyStyle: "padding: 0px;"}});
    return false;
  },
  toList: function(num) {
  ...
  ttActGift: function(uid) {
    return !showBox('al_gifts.php', {act: 'get_gift_box', mid: uid, fr: (uid == vk.id ? 1 : 0)}, {stat: ['gifts.css', 'ui_controls.js', 'ui_controls.css'], cache: 1}, false);
  },
  ...
  editList: function(listId) {
    ...
    cur.onFlistSave = function (ids, list, hash, title) {
      ...
      ajax.post('al_friends.php', {act: 'save_list', title: title, cat_id: listId, Friends: friendsList.join(','), hash: hash}, {
        ...
        onFail: function(text) {
          setTimeout(showFastBox({title: getLang('global_error')}, text, getLang('global_close')).hide, 4000);
          return true;
        }
      });
  ...
  deleteList: function(listId) {
    showBox('al_friends.php', {act: 'delete_list_box', list_id: listId});
  },
  ...
  changeFilter: function() {
    ...
    if (cur.section == 'suggestions') {
      return Friends.section(cur.section, function() {}, cur.filter || true);
    }
    if (cur.filter) {
...
extend(Friends, {
  ...
  deleteRow: function(row, hash) {
    ...
    var list = cur.friendsList['suggestions'];
    row = row.split('_');
    for (var i in list) {
      if (parseInt(list[i][0]) == parseInt(row[1])) {
        cur.friendsList['suggestions'].splice(i, 1);
      }
    }
    cur.importCount -= 1;
  }
