var Join = {
  ...
  showRows: function(fromStart, rows, from, preload) {
    ...
    show(cur.resultEl);
    hide(cur.imgEl);
    val(cur.noneEl, getLang('join_select_' + cur.section + '_after'));
      ...
      show: function(el) {
        setTimeout(function() {
          if (!intval(cur.uiCity.val())) {
            Join.tipShow(cur.section + '_city_tip', 'join_' + cur.section + '_city_tip', [-15, -52, 3], 'join_phone_tt');
          }
        }, 0);
        Join.show(el);
      },
  ...
  focusWithoutDropdown: function() {
    ...
    dd.focus();
    dd.select.hide();
    Join.tipShow(cur.section + '_' + cur.section + '_tip', 'join_' + cur.section + '_tip', [-15, (cur.section == 'school') ? -52 : -36, 3], 'join_' + cur.section + '_tt');
  },
  yearTip: function() {
    if (intval(cur.uiYear.val())) {
      Join.tipHide(cur.section + '_year_tip');
    } else {
      Join.tipShow(cur.section + '_year_tip', 'join_' + cur.section + '_year_tip', [-15, -36, 3], 'join_year_tt');
    }
  },
  ...
  submitPhone: function(force) {
    ...
    var phoneEl = ge('join_phone'), phone = Join.getPhone(), phoneInputVal = val('join_phone'), email = val('join_email'), cntr = cur.uiPhoneCountry ? cur.uiPhoneCountry.val_full() : false;
    if (phoneEl.disabled) return elfocus('join_code');

    ajax.post('join.php', {act: 'phone', phone: phone, email: email, hash: cur.hash}, {
      showProgress: lockButton.pbind('join_send_phone'),
      hideProgress: unlockButton.pbind('join_send_phone'),
      onDone: function(needCheck, boxBody, strong) {
        Join.askPassword('','');
        if (needCheck) {
  ...
  updateStrength: function() {
    var pwd = val('join_pass'), level = -1, pwds = ['qwerty', 'йцукен', 'gfhjkm', 'пароль', 'password', 'abc123', 'fuckyou', '123abc', 'baseball', 'football', 'soccer', 'monkey', 'liverpool', 'princess', 'jordan', 'slipknot', 'superman', 'iloveyou'];
    ...
    if (pwd.length < 6) {
      ...
      var groups = [/[^a-z]/g, /[^0-9]/g], cnts = [], big = 0;
...
'}));
  ...
  submitPassword: function(toAlready, fbLoginTo) {
    ...
    onLoginFailed = function(code) {
      if (code === -1) {
        ...
        ajax.post('join.php', {act: 'done', pass: pass, email: email, phone: login}, {onDone: function() {
  ...
  noCode: function() {
    var prg = ce('span', {className: 'progress_inline'}), el = ge('join_resend'), prnt = el.parentNode;
