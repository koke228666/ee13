var Page = {
  ...
  postsSeen: function(posts, oid) {
    ...
    if (ch) {
      ...
      _postsSaveTimer = setTimeout(Page.postsSave, 5000);
  ...
  postsSave: function() {
    ...
    var ch, i, p, snt, sn, sent = ls.get('posts_sent') || {}, seen = ls.get('posts_seen') || {};
    for (i in _postsSeen) {
      sn = _postsSeen[i];
      p = i.split('_');
      p[0] = intval(p[0]);
  ...
  postsSend: function() {
    ...
    ajax.post('al_page.php', {act: 'seen', data: data.join(';')}, {onDone: function() {
      ...
      var cseen = ls.get('posts_seen') || {}, sent = ls.get('posts_sent') || {}, i, j;
      for (i in seen) {
        for (j in seen[i]) {
          if (!sent[i]) {
            sent[i] = {};
  ...
  editContact: function(oid, mid, hash, callback) {
    var b = showBox('al_page.php', {act: 'a_edit_contact_box', mid: mid, oid: oid}).setButtons(getLang('global_save'), function() {
  ...
  deleteContact: function(oid, mid, hash) {
    cur.reloadAfterClose = true;
    ajax.post('al_page.php', {act:'a_delete_contact', oid: oid, mid: mid, hash:hash}, {onDone: function(res){
      curBox().content(res);
      toggle('public_add_contact', ge('public_contacts_list').childNodes.length < 30);
    }});
  },
  ...
  infoSave: function(txt) {
    ...
    ajax.post('page.php', {act: 'current_info', oid: cur.oid, info: txt, hash: cur.options.info_hash}, {onDone: function() {
  ...
  showGif: function(obj, ev, doc, hash, addTxt) {
    ...
    var acts = canAdd ? '<div class="page_gif_add" onmouseover="return Page.overGifAdd(this, \'' + addTxt + '\', \''+doc+'\', event);" onmouseout="Page.outGifAdd(this, event);" onclick="return Page.addGif(this, \''+doc+'\', \''+hash+'\', event);"><div class="page_gif_add_icon"></div></div>' : '';
...
var Wall = {
  ...
  sendReply: function(post) {
    ...
    newEl = se(rs(cur.wallTpl.reply_fast, {
      reply_id: '0_' + replyId,
      message: clean(params.message).replace(/\n/g, '<br/>'),
...
function initCustomMedia(lnk, types, opts) {
  ...
  var html = '<div class="rows"><div class="add_media_head noselect"><nobr>' + lnk.innerHTML + '</nobr></div></div>';
  ...
  var _hideTimer, mediaMenu = {
    ...
    setItems: function(types) {
      for (var f = rowsNode.firstChild, l = rowsNode.lastChild; f != l; f = rowsNode.firstChild, l = rowsNode.lastChild) {
        rowsNode.removeChild(hasClass(f, 'add_media_head') ? l : f);
      }
      var test = '';
    ...
    switch (v[0]) {
      ...
      case 'gift':
        handler = function() {
          ...
          cur.giftSendFrom = 'im';
          showBox('al_gifts.php', {act: 'get_gift_box', mid: mid, fr: (mid == vk.id ? 1 : 0)}, {stat: ['gifts.css', 'wide_dd.js', 'wide_dd.css']});
        }
        break;
    }
    var isApp = (v[0] == 'app');
...
Composer = {
  ...
  onKeyEvent: function (composer, event) {
    ...
    if (event.type == 'keypress' || event.type == 'keydown') {
      ...
      if (controlEvent && isVisible(composer.wdd.listWrap)) {
