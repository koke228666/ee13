...
videochatBox: function(title) {
  ...
  var box = defBox({
    ...
    y: size.y + 'px',
    resize: true,
    width: 500, height: size.w,
    minW: 320, minH: size.h,
    onResize: function(h, w) {
      if (Call.videochat) {
  ...
  setStyle(header, 'padding', '5px 0px');
  var tt = geByClass1('fc_tab_title', header, 'div');
  setStyle(tt, {color: '#888', fontSize: '11px'});
  addClass(tt, 'fl_r');
...
incomingReply: function(call_id, uid, farId, hash, stream, rtmp, rtmfp, prefer, video) {
  ...
  Call.dismissInvitation();
  setFavIcon('/images/icons/fav_call.ico');
  Call.params = {
    call_id: call_id,
    from_id: uid,
    far_stream: stream,
