var ProfileEditor = {
  go: function(el, ev) {
    if (ProfileEditor.checkChanges(1) === false) {
      cur.onContinueCb = ProfileEditor.go.pbind(el, false);
      return false;
    }
      ...
      case 'career':
        ...
        for (var i = 0; i < cur.works.length; ++i) {
          ...
          if (cur.works[i].id > 0 && ProfileEditorJob.workChanged(cur.works[i])) {
            work_change = true;
          } else if (cur.works[i].id < 0 && isVisible('content' + cur.works[i].id) && (ge('company' + cur.works[i].id).value.length || position.length)) {
            work_change = true;
          }
        }
        return work_change;
